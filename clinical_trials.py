...
def clinical_trials_by_company_update_storage(start_date: str|None=None):
    if start_date is None:
        database_url = f"mysql+pymysql://root:{news_database}@127.0.0.1:3306/healthcare"
        engine = create_engine(database_url, pool_pre_ping=True, connect_args={"connect_timeout": 5})
        df = pd.read_sql_table("clinical_trials_by_company", engine)
        last_ts = pd.to_datetime(df["Last_Update_Posted_Date"], errors="coerce").max()
    ...
    for c in str_cols:
        # If the column might contain non-strings, normalize to string first (preserving NULLs)
        _df[c] = _df[c].astype("string")            # pandas StringDtype supports <NA>
        _df[c] = _df[c].str.strip()                 # trim leading/trailing whitespace
        _df[c] = _df[c].replace({"": pd.NA})        # empty -> NA (later becomes SQL NULL)
